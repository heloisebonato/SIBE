...
const LocacaoEditar = (props) => {
  ...
  const [carretasBloqueadas, setCarretasBloqueadas] = useState(() => {
    (async () => {
      ...
      data.forEach(function (arrayItem) {
        ...
        for (
          var arr = [], dt = new Date(arrayItem.data_prevista_saida);
          dt <= new Date(arrayItem.data_prevista_entrada);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await axios.get(
        `locacoes/${props.match.params.locacao_id}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      window.confirm("Você tem certeza que deseja editar este Agendamento?")
    ) {
      var today = new Date();
  ...
  const checarDatas_saida = async (e) => {
    ...
    var data_prevista_saida_s =
      data_prevista_saida_d.getFullYear() +
      "-" +
      (data_prevista_saida_d.getMonth() + 1) +
      "-" +
      data_prevista_saida_d.getDate();
    ...
    carretasBloqueadas.forEach(function (arrayItem) {
      if (
        ...
        arrayItem.arrayDatasBloqueadas.forEach(function (dataItem) {
          console.log(
            "DATA: " + dataItem + "DATA_PRETENDIDA: " + data_prevista_saida_d
          );
          var data_alugada =
    ...
    if (data_prevista_saida_d < today) {
      ...
      window.confirm(
        "Data Bloqueada por Outro Agendamento Verificar Calendário."
      );
    } else {
  ...
  const checarDatas_entrada = async (e) => {
    ...
    var data_prevista_entrada_s =
      data_prevista_entrada_d.getFullYear() +
      "-" +
      (data_prevista_entrada_d.getMonth() + 1) +
      "-" +
      data_prevista_entrada_d.getDate();
    ...
    if (data_prevista_entrada_d < today) {
      ...
      window.confirm(
        "Data de Retorno precisa ser maior ou igual a data de Saída."
      );
      document.getElementById("data_prevista_entrada").value = "";
    } else {
